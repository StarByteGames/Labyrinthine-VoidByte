...
namespace VoidByte.Utilities
{
    internal class Drawing
    {
        ...
        )
        {
            ...
            GUI.Label(
                new Rect(centered ? (position - vector / 2f) : position, vector),
                content,
                Drawing.StringStyle
            );
        }
        ...
        public static void TextWithDistance(Transform target, string text, Camera relativeTo = null)
        {
            ...
            {
                float distanceToMonster = (float)
                    Math.Round(
                        (double)
                            Vector3.Distance(
                                Core.PlayerControl.transform.position,
                                target.position
                            ),
                        1
                    );
                ...
                if (vector.z >= 0f && distanceToMonster < 125f)
                {
                    DrawString(
                        ...
                        text + " [" + distanceToMonster.ToString() + "m]",
                        Color.cyan,
                        12,
                        true
                    );
