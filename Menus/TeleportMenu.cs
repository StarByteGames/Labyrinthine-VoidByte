...
namespace VoidByte.Menus
{
    public class TeleportMenu : MonoBehaviour
    {
        ...
        public static void Render(int windowID)
        {
            ...
            foreach(var player in Core.GameManager.AlivePlayers)
            {
                if (Core.PlayerControl.playerNetworkSync.playerName == player.playerName)
                {
                    continue;
                }
                ...
                {
                    Core.PlayerControl.playerNetworkSync.MoveToTransform(player.transform);
                }
            }
            ...
            {
                Core.PlayerControl.playerNetworkSync.MoveToSpawnpoint();
            }
        ...
        private static void RenderMonstersColumn()
        {
            ...
            foreach (var ai in Core.AIControllers)
            {
                ...
                if (GUI.Button(new Rect(316f, monsterColY, 127f, 23f), new GUIContent(normalizedMonsterName)))
                {
                    ...
                    Core.PlayerControl.playerNetworkSync.MoveToTransform(ai.transform);
                }
