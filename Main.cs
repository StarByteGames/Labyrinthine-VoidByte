...
namespace VoidByte
{
    public class Core : MelonMod
    {
        ...
        public override void OnGUI()
        {
            ...
            {
                LobbyMenu.MenuRect = GUI.Window(
                    0,
                    LobbyMenu.MenuRect,
                    (GUI.WindowFunction)LobbyMenu.Render,
                    "~ Labyrinthine Hack ~"
                );
            }
            ...
            {
                InGameMainMenu.MenuRect = GUI.Window(
                    1,
                    InGameMainMenu.MenuRect,
                    (GUI.WindowFunction)InGameMainMenu.Render,
                    "~ Labyrinthine Hack ~"
                );

                if (TeleportMenu.Enabled)
                {
                    TeleportMenu.MenuRect = GUI.Window(
                        2,
                        TeleportMenu.MenuRect,
                        (GUI.WindowFunction)(TeleportMenu.Render),
                        "~ Teleport Menu ~"
                    );
                }
            }
